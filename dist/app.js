...
const state = {
  ...
  appearance: APPEARANCE_MODES.includes(readStorageValue("appearance", "system")) ? readStorageValue("appearance", "system") : "system",
  viewMode: "grid",
  group: readStorageValue("nodeSelectedGroup", "__all__") || "__all__",
  query: "",
  searchOpen: false,
  detailNodeId: null,
  detailSection: "load",
...
function populateNodeCard(root, node) {
  ...
  const regionMarkup = renderRegionFlag(node);
  if (shouldShowFlag() && regionMarkup) {
    flag.innerHTML = regionMarkup;
  ...
  root.querySelector(".dials").innerHTML = [
    dialGroup(t().cpu, safeNumber(live?.cpu?.usage), online ? `${safeNumber(live?.cpu?.usage).toFixed(1)}%` : t().offline),
    dialGroup(t().memory, percentValue(live?.ram?.used, live?.ram?.total || node.mem_total), online ? formatBytes(live?.ram?.used) : t().offline),
    dialGroup(t().disk, percentValue(live?.disk?.used, live?.disk?.total || node.disk_total), online ? formatBytes(live?.disk?.used) : t().offline)
  ].join("");
...
function renderLatencyLine(ping, online = true) {
  const latency = online ? (ping.average ? formatLatency(ping.average) : t().loading) : "-1 ms";
  const loss = online ? formatPercent(ping.loss) : "0.0%";
  const jitter = online ? formatLatency(ping.jitter) : "0.0 ms";
...
function renderDetailPage(includeCharts = true) {
  ...
  const regionMarkup = renderRegionFlag(node);
  if (shouldShowFlag() && regionMarkup) {
    elements.detailFlag.innerHTML = regionMarkup;
    elements.detailFlag.classList.remove("hidden");
  } else {
    elements.detailFlag.innerHTML = "";
    elements.detailFlag.classList.add("hidden");
  }
...
function getPingOverview(uuid, hours) {
  const entry = PING_CACHE.get(`${uuid}:${hours}`)?.data;
  if (!entry) return { average: 0, loss: 0, jitter: 0 };
  const overview = summarizePing(entry);
  return { average: overview.average, loss: overview.loss, jitter: overview.jitter };
}
...
function cycleAppearance() {
  ...
  writeStorageValue("appearance", next);
  applyAppearance();
  renderChrome();
  if (state.detailNodeId) renderDetailPage();
  else renderNodes();
...
function normalizeSharedStorage() {
  [
    ["nodeSelectedGroup", state.group],
    ["nodeViewMode", state.viewMode],
    ["i18nextLng", state.language]
  ].forEach(([key, fallback]) => {
    const value = readStorageValue(key, fallback);
    writeStorageValue(key, value);
  });
  if (hasStorageValue("appearance")) {
    writeStorageValue("appearance", resolveAppearanceMode(readStorageValue("appearance", state.appearance)));
  }
}
...
function renderRegionFlag(input) {
  ...
  if (code) return `<img class="flag-image" src="${escapeAttribute(flagAssetUrl(code))}" alt="${escapeAttribute(code.toUpperCase())}" loading="lazy" decoding="async" referrerpolicy="no-referrer" crossorigin="anonymous">`;
